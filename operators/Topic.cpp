...
/*   Updated: 2025/03/06 22:24:08 by ael-fagr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (!My_serv.getClients().empty())
    {
        std::string str = RPL_TOPIC(My_serv.getClients()[Client_id - 1].getNickname(), channel.getChannelName(), topic);
...
void Desplay_topic(Server &My_serv, Channel  &channel, int Client_id){
    ...
    if (My_serv.getChannels().empty() || My_serv.getClients().empty())
        return ;
    std::string str =  RPL_TOPIC(My_serv.getClients()[Client_id - 1].getNickname(), channel.getChannelName(), channel.getTopic());
...
int Operators::Topic_func(Server &My_serv, std::string arg, int Client_id)
{
    ...
    else
    {
        ...
        if (op.there_is_channel(My_serv, channel, channel_pos, Client_id)){
            if (channel_pos == -1)
                return (1);
            op.Add_topic(My_serv, My_serv.getChannels()[channel_pos], topic, Client_id);
        }
